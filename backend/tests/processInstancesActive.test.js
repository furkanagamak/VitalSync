...
describe("GET /processInstancesActive", () => {
  ...
  it("usual GET", async () => {
    const activeProcessRes = await request(app).get(`/processInstancesActive`);
    expect(activeProcessRes.status).toBe(200);

    const exisitingProcess = await ProcessInstance.findOne({
      processID: "TPID-123",
    });

    if (!exisitingProcess)
      throw new Error("Please run dbtest.js before running this test");

    const expectedData = [
      {
        objectID: exisitingProcess._id.toString(),
        processID: "TPID-123",
        processName: "ProcessName",
        description: "Description",
        patientFullName: "Alice Smith",
      ...
      {
        objectID: testProcess._id.toString(),
        processID: "AB12CD34",
        processName: "Radical Prostatectomy",
        description: "Surgical removal of the prostate gland",
        patientFullName: "Alice Johnson",
        procedures: [
          "Preoperative Theorapy",
          "Pelvic Ultrasound",
          "Anesthesia Shot",
          "Prostate Removal",
        ...
        nextProcedure: "Prostate Removal",
      },
      {
        objectID: testProcess2._id.toString(),
        processID: "TPID-12312312",
        processName: "Routine Checkup",
        description: "Regular health assessment and tests",
    ...
    expect(activeProcessRes.body).toEqual(expectedData);
  });
