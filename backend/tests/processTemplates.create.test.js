...
describe("POST /processTemplates", () => {
    ...
    it("should create a process template successfully with predefined procedures", async () => {
      ...
      const role = await new Role({ name: "Neurosurgeon", uniqueIdentifier: "NES-02"}).save();
      const resource = await new ResourceTemplate({ type: "Equipment", name: "Scalpel Kit" }).save();
