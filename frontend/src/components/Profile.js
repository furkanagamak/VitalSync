...
function ImageUploader({ onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center" onClick={onClose}>
      ...
      >
        <h2 className="text-base leading-6 text-center text-black">Choose Image to Upload</h2>
        ...
        <div className="flex items-center gap-2 self-stretch mt-1.5 text-xs text-center text-black">
          <label className="justify-center px-2 py-1.5 rounded-sm border border-black border-solid bg-zinc-300 cursor-pointer">
            Choose File
            <input type="file" accept=".png, .jpeg, .gif" onChange={handleFileSelection} style={{ display: 'none' }} />
          </label>
          {fileName && <span className="text-sm text-neutral-700">{fileName}</span>}
        </div>
        <div className="flex gap-5 mt-3.5 justify-center">
          <button className="flex-1 cursor-pointer border border-solid border-neutral-600 text-neutral-600 text-center py-1.5 rounded-lg" onClick={onClose}>Cancel</button>
          <button className="flex-1 cursor-pointer bg-red-800 text-white text-center py-1.5 rounded-lg" onClick={onClose}>Upload</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex items-center justify-center min-h-screen">
      <div className="text-center">
        ...
        <button onClick={handleReturnToRoster} className="px-4 py-2 bg-blue-500 text-white rounded">
    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center">
      <div className="flex flex-col justify-center max-w-[364px] bg-lime-50 rounded-lg border border-red-800 border-solid shadow">
        <p className="text-base leading-6 text-center text-black px-16 py-6">
          ...
          <br />The temporary password is:
          <br /><span className="font-bold">B6wcCW53</span>
          <br />A confirmation Email has been sent
        </p>
        <button type="button" onClick={onClose} className="justify-center items-center self-center px-5 py-1.5 mt-1 mb-4 text-xs bg-white rounded-lg border border-solid border-neutral-600 text-neutral-600">Close</button>
      </div>
    </div>
  );
}
    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center">
      <div className="flex flex-col justify-center max-w-[364px] bg-lime-50 rounded-lg border border-red-800 border-solid shadow">
        ...
        <div className="flex gap-5 mt-7 text-sm font-medium justify-center px-5 pb-5">
          <button onClick={onClose} className="flex-1 justify-center px-5 py-3.5 bg-white rounded-lg border border-solid border-neutral-600 text-neutral-600">No</button>
          <button onClick={onConfirm} className="flex-1 justify-center px-5 py-3.5 bg-red-800 rounded-lg text-white">Yes</button>
        </div>
      </div>
    </div>
  );
      ...
      <div className="flex flex-col justify-center max-w-[436px]">
        <div className="flex flex-col items-center px-7 pt-3.5 pb-7 w-full bg-lime-50 rounded-lg border border-red-800 border-solid shadow">
          <div className="self-stretch text-base leading-6 text-center text-black">
            Are you sure you want to terminate this account? If yes, write the person's name of the account to be terminated.
          </div>
          <input
            type="text"
            value={name}
          ...
          <div className="flex gap-5 mt-4 text-sm font-medium whitespace-nowrap">
            <button onClick={onClose} className="flex-1 justify-center px-5 py-3 bg-white rounded-lg border border-solid border-neutral-600 text-neutral-600">Cancel</button>
            <button onClick={handleSubmit} className="flex-1 justify-center px-5 py-3 bg-red-800 rounded-lg text-white">Submit</button>
          </div>
        </div>
      </div>
    </div>
...
function ProfileImage() {
  ...
  return (
    <div className="flex flex-col self-stretch my-auto text-sm font-medium text-neutral-600 max-md:mt-10">
      ...
      <div className="justify-center self-center p-1 mt-3.5 rounded-lg border border-solid bg-zinc-300 border-neutral-600 cursor-pointer" onClick={() => setShowUploader(true)}>
...
function ContactInfo() {
  ...
  return (
    <div className="flex flex-col mt-1.5 text-3xl text-black max-md:mt-10">
      ...
      {editMode ? (
        <>
          <input type="text" value={cellNo} onChange={(e) => setCellNo(e.target.value)} className="mt-6 text-left border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent" />
          <input type="text" value={officeNo} onChange={(e) => setOfficeNo(e.target.value)} className="mt-3 text-left border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent" />
          <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="mt-2.5 text-left border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent" />
          <input type="text" value={office} onChange={(e) => setOffice(e.target.value)} className="mt-2 text-left border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent" />
        </>
      ) : (
        <>
          <p className="mt-6 text-left"><span className="text-red-800">Cell No</span>: {cellNo}</p>
          <p className="mt-3 text-left"><span className="text-red-800">Office No</span>: {officeNo}</p>
          <p className="mt-2.5 text-left"><span className="text-red-800">Email</span>: {email}</p>
          <p className="mt-2 text-left"><span className="text-red-800">Office</span>: {office}</p>
        </>
      )}
      <div className="flex gap-5 justify-between items-start mt-24 text-sm font-medium text-neutral-600 max-md:pr-5 max-md:mt-10">
        <button onClick={() => setEditMode(!editMode)} className="justify-center px-1.5 py-1 rounded-lg border border-solid bg-zinc-300 border-neutral-600">{editMode ? "Save Changes" : "Change Contact Info"}</button>
        <button onClick={handleResetPasswordClick} className="justify-center px-2 py-1 rounded-lg border border-solid bg-zinc-300 border-neutral-600">Reset Password</button>
      </div>
      {showResetPasswordModal && <ConfirmResetPasswordModal onClose={() => setShowResetPasswordModal(false)} onConfirm={handleResetPasswordConfirm} />}
      {showPasswordResetConfirmation && <PasswordResetConfirmation onClose={() => setShowPasswordResetConfirmation(false)} />}
    </div>
  );
}
...
function ProfileDetails() {
  ...
  const [departmentHead, setDepartmentHead] = useState("Head of the Neurology Department");
  ...
  return (
    <div className="flex flex-col grow gap-4 border-r border-black max-md:flex-wrap max-md:mt-10">
      <div className="flex flex-col grow shrink-0 self-start basis-0 w-full max-md:max-w-full">
        {editMode ? (
          <>
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="mb-2 text-5xl text-red-800 max-md:text-4xl text-left border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent" />
            <input type="text" value={designation} onChange={(e) => setDesignation(e.target.value)} className="mb-2 text-3xl text-black text-left border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent" />
            <input type="text" value={specialty} onChange={(e) => setSpecialty(e.target.value)} className="mb-2 text-3xl text-left text-black max-md:max-w-full border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent" />
            <input type="text" value={department} onChange={(e) => setDepartment(e.target.value)} className="mb-2 text-3xl text-left text-black max-md:max-w-full border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent" />
            <input type="text" value={departmentHead} onChange={(e) => setDepartmentHead(e.target.value)} className="mb-2 text-3xl text-left text-black max-md:max-w-full border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent" />
          </>
        ) : (
          <>
            <h1 className="mb-2 text-5xl text-red-800 max-md:text-4xl text-left">{name}</h1>
            <p className="mb-2 text-3xl text-black text-left">{designation}</p>
            <p className="mb-2 text-3xl text-left text-black max-md:max-w-full">{specialty}</p>
            <p className="mb-2 text-3xl text-left text-black max-md:max-w-full">{department}</p>
            <p className="mb-2 text-3xl text-left text-black max-md:max-w-full">{departmentHead}</p>
          </>
        )}
      </div>
      <button onClick={() => setEditMode(!editMode)} className="px-5 py-1 text-sm font-medium text-neutral-600 bg-zinc-300 border border-solid border-neutral-600 rounded-lg self-start mt-auto">{editMode ? "Save Changes" : "Edit Profile"}</button>
    </div>
  );
}
  ...
  return (
    <div >
      ...
      <img
        ...
        />
        <button onClick={onScheduleChange} className="justify-center px-1.5 py-1 rounded-lg border border-solid bg-zinc-300 border-neutral-600">
          Change Schedule
        </button>
      </div>
...
function MyComponent() {
  ...
  return (
    <div className="flex flex-col items-center pt-10 pr-5 pb-8 pl-14 bg-white max-md:pl-5">
      <button onClick={handleTerminateAccount} className="justify-center self-end px-3 py-1 text-sm font-medium text-white bg-red-800 rounded-lg border border-solid border-neutral-600">
        Terminate Account
      </button>
      ...
      <div className="mt-4 w-full max-w-[1286px] max-md:max-w-full">
        ...
        {showTerminationModal && <AccountTerminationModal onClose={handleCloseTerminationModal} onTerminate={handleTerminationConfirmation} />}
      </div>
    </div>
  );
}
